...
#define STD_ITERATOR_ENUM_GET_AND_CHECK_SUB(IT,IMPLEMENTS,IMPVAR,GETVAR) \
	(	\
		(void) sizeof(enum STD_FAKEVAR() {	\
			IMPVAR = (int) STD_ITERATOR_PARENT_IMPLEMENTS_GET(IT), \
			GETVAR = (int) STD_ITERATOR_ENUM_GET(IT) \
		}),		\
		STD_ITERATOR_CALL_EXISTS(IT,IMPLEMENTS,IMPVAR,GETVAR), \
		GETVAR	\
	)
